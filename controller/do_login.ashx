...
public class do_login : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        MySqlCommand hCommand = new MySqlCommand("insert into zdb_user(username,password,salt) values('zdb7','zdb5','zdb5')",hConnect);
        hConnect.Open();
        ...
        try {
            ...
            while (result.Read())
            {
                if (result.HasRows)
                {
                    ...
                    context.Response.Write(JsonConvert.SerializeObject(hConnect));
                }
            }
